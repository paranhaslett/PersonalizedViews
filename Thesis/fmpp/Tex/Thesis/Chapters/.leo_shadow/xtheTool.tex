...
\begin{description}
  ...
  Once a difference has been detected we attempt to remove any of the information we are not going to use in the future.  This should help the garbage collecter free up memory. We do this by setting the AST nodes recorded against a difference to null once the difference has been discovered. 
  %@nonl
  %@-node:paran.20140806072550.2155:<<setting to null>>
  %@nl
  ...
  By ignoring AST nodes and any of their child nodes that do not have a text based change comparing all the AST nodes can be avoided.  This will also help to speed up anaylisis as these nodes do not need to be analysed to see if they match.
  %@nonl
  %@-node:paran.20140805192059.2163:<<not analysing>>
  %@nl
